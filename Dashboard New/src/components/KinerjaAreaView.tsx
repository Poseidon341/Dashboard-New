...
import { UkerInfo, RawSegmenRow, TopRankItem } from '../types';
...
interface KinerjaAreaViewProps {
...
export const KinerjaAreaView: React.FC<KinerjaAreaViewProps> = ({
  ukerMap,
  rkaMap,
  rawSegmenData,
  selectedArea,
  ...
  // Extract Day number (1..31) for active marker in charts
  ...
  const dynamicTitle = useMemo(() => {
    let title = 'Dashboard Kinerja Area';
  ...
  const computed = useMemo(() => {
    if (!selectedDate || rawSegmenData.length === 0 || Object.keys(ukerMap).length === 0) {
      return {
        totals: {
        ...
        cabangData: {
          tabungan: {} as Record<string, number>,
          giro: {} as Record<string, number>,
          deposito: {} as Record<string, number>,
          dpk: {} as Record<string, number>,
        },
        chartData: {
          dpk: initSeries(),
          tabungan: initSeries(),
    ...
    const cabangData: Record<string, Record<string, number>> = {
      tabungan: {},
      giro: {},
      deposito: {},
      dpk: {},
    ...
    // Calculate Targets from RKA Map
    Object.keys(rkaMap).forEach((uker) => {
      const ukerInfo = ukerMap[uker];
      const cleanCabang = normalizeCabangName(ukerInfo?.cabang || '');
      const branchArea = ukerInfo?.area?.trim() || getAreaForCabang(cleanCabang);

      if (selectedArea !== 'ALL' && branchArea !== selectedArea) return;
      if (selectedCabang !== 'ALL' && cleanCabang !== selectedCabang) return;

      const rka = rkaMap[uker];
      if (rka) {
        if (rka.tabungan && rka.tabungan[monthIndex]) totals.tabungan.target += rka.tabungan[monthIndex];
        if (rka.giro && rka.giro[monthIndex]) totals.giro.target += rka.giro[monthIndex];
        if (rka.deposito && rka.deposito[monthIndex]) totals.deposito.target += rka.deposito[monthIndex];
        if (rka.dpk && rka.dpk[monthIndex]) totals.dpk.target += rka.dpk[monthIndex];
      }
    });

    // Process Raw Daily Rows
    rawSegmenData.forEach((row) => {
      ...
      if (rowDateStr === t_str) {
        ...
        cabangData[prodKey][cleanCabang] = (cabangData[prodKey][cleanCabang] || 0) + saldo;
      ...
      if (cArrayKey && dt >= 0 && dt < 31) {
        if (cArrayKey !== 'aug26' || dt < selectedDay) {
          const prevProd = chartData[prodKey][cArrayKey][dt] || 0;
          chartData[prodKey][cArrayKey][dt] = prevProd + saldo;

          const prevDpk = chartData.dpk[cArrayKey][dt] || 0;
          chartData.dpk[cArrayKey][dt] = prevDpk + saldo;
        }
      }
    });
  ...
  const insight = useMemo(() => {
    ...
    return {
      segmenLabel: selectedSegmen === 'ALL' ? 'Total Konsolidasi' : selectedSegmen,
      dpkPct,
      isSurplus,
      surplusAmount,
      mtdDelta,
  ...
  const topRankings = useMemo(() => {
    const sortRanking = (obj: Record<string, number>, asc = false): TopRankItem[] => {
      return Object.keys(obj)
        .map((cab) => ({ cabang: cab, saldo: obj[cab] }))
        .sort((a, b) => (asc ? a.saldo - b.saldo : b.saldo - a.saldo))
        .slice(0, 5);
    };

    return {
      tabHigh: sortRanking(computed.cabangData.tabungan, false),
      tabLow: sortRanking(computed.cabangData.tabungan, true),
      giroHigh: sortRanking(computed.cabangData.giro, false),
      giroLow: sortRanking(computed.cabangData.giro, true),
      depHigh: sortRanking(computed.cabangData.deposito, false),
      depLow: sortRanking(computed.cabangData.deposito, true),
      dpkHigh: sortRanking(computed.cabangData.dpk, false),
      dpkLow: sortRanking(computed.cabangData.dpk, true),
    };
  }, [computed.cabangData]);
  ...
  const handleExportCSV = () => {
    ...
    const list = [
      { name: 'DPK (Total)', data: computed.totals.dpk },
    ...
    link.setAttribute('download', `Kinerja_Area_${selectedDate}.csv`);
  ...
  const renderRankingList = (items: TopRankItem[], color: string, isHigh = true) => {
    if (items.length === 0) {
      return <li style={{ justifyContent: 'center', color: '#999', fontStyle: 'italic' }}>Data belum tersedia</li>;
    }
  ...
  return (
    <div id="page-area" className="page-section active-page">
      ...
      <div className="summary-grid">
        ...
        <div className="sum-card" style={{ borderTopColor: '#0857C3' }}>
          <div className="sum-card-header">
            <h4 style={{ color: '#0857C3' }}>DPK (Total)</h4>
        ...
        <div className="sum-card" style={{ borderTopColor: '#FF8F00' }}>
          <div className="sum-card-header">
            <h4 style={{ color: '#FF8F00' }}>Tabungan</h4>
        ...
        <div className="sum-card" style={{ borderTopColor: '#05CD99' }}>
          <div className="sum-card-header">
            <h4 style={{ color: '#05CD99' }}>Deposito</h4>
      ...
      <div className="quick-insight-container">
        <h3>
          <i className="fa-solid fa-lightbulb" style={{ color: '#FF8F00' }}></i> Quick Insight
        </h3>
        <div className="quick-insight-box">
          Kinerja DPK <strong>{insight.segmenLabel}</strong> telah mencapai{' '}
          <strong>{insight.dpkPct}%</strong> dari target dengan {insight.isSurplus ? 'surplus' : 'defisit'}{' '}
          <strong>Rp {insight.surplusAmount}</strong>.
          Momentum bulan ini tercatat <strong>{insight.isMtdPositive ? 'positif 📈' : 'negatif 📉'}</strong>, yaitu MTD{' '}
          {insight.mtdDelta < 0 ? '-' : '+'}Rp {formatUangShort(Math.abs(insight.mtdDelta))} (
          {insight.mtdDelta > 0 ? '+' : ''}
          {insight.mtdPct}%). Secara fundamental, kinerja secara keseluruhan masih{' '}
          <strong>{insight.isYtdPositive ? 'tumbuh 🚀' : 'menurun'}</strong>, dengan YTD tercatat{' '}
          <strong>
            {insight.ytdDelta >= 0 ? '+' : ''}
            {insight.ytdPct}%
          </strong>.
      ...
      {/* DAILY TIMESERIES PRESENTATION SLIDE - 4 CHARTS GRID SEPERTI GAMBAR KE-2 */}
      <div className="timeseries-presentation-wrapper">
        ...
        <div className="timeseries-header-banner">
          ...
          <div className="timeseries-title">
            Daily TimeSeries : as of {formattedBannerDate}
          </div>
        </div>

        {/* 4 Charts Grid (Tabungan, Giro, Deposito, DPK Total) */}
        <div className="charts-grid-4">
          <TrendTimeSeriesChart
            id="tabungan"
            title="Trend Saldo Tabungan"
            themeColor="#ea580c"
      ...
      <div className="top5-grid-4">
        <div className="top5-card">
          <div className="top5-header" style={{ color: '#FF8F00' }}>
            <i className="fa-solid fa-arrow-trend-up"></i> TOP 5 Tertinggi - Tabungan
          </div>
          <ul className="top5-list">{renderRankingList(topRankings.tabHigh, '#2b3674', true)}</ul>
        </div>

        <div className="top5-card">
          <div className="top5-header" style={{ color: '#307FE2' }}>
            <i className="fa-solid fa-arrow-trend-up"></i> TOP 5 Tertinggi - Giro
          </div>
          <ul className="top5-list">{renderRankingList(topRankings.giroHigh, '#2b3674', true)}</ul>
        </div>

        <div className="top5-card">
          <div className="top5-header" style={{ color: '#05CD99' }}>
            <i className="fa-solid fa-arrow-trend-up"></i> TOP 5 Tertinggi - Deposito
          </div>
          <ul className="top5-list">{renderRankingList(topRankings.depHigh, '#2b3674', true)}</ul>
        </div>

        <div className="top5-card">
          <div className="top5-header" style={{ color: '#0857C3' }}>
            <i className="fa-solid fa-arrow-trend-up"></i> TOP 5 Tertinggi - DPK
          </div>
          <ul className="top5-list">{renderRankingList(topRankings.dpkHigh, '#2b3674', true)}</ul>
        </div>
      </div>
      ...
      <div className="top5-grid-4">
        <div className="top5-card">
          <div className="top5-header" style={{ color: '#EE5D50' }}>
            <i className="fa-solid fa-arrow-trend-down"></i> TOP 5 Terendah - Tabungan
          </div>
          <ul className="top5-list">{renderRankingList(topRankings.tabLow, '#ee5d50', false)}</ul>
        </div>

        <div className="top5-card">
          <div className="top5-header" style={{ color: '#EE5D50' }}>
            <i className="fa-solid fa-arrow-trend-down"></i> TOP 5 Terendah - Giro
          </div>
          <ul className="top5-list">{renderRankingList(topRankings.giroLow, '#ee5d50', false)}</ul>
        </div>

        <div className="top5-card">
          <div className="top5-header" style={{ color: '#EE5D50' }}>
            <i className="fa-solid fa-arrow-trend-down"></i> TOP 5 Terendah - Deposito
          </div>
          <ul className="top5-list">{renderRankingList(topRankings.depLow, '#ee5d50', false)}</ul>
        </div>

        <div className="top5-card">
          <div className="top5-header" style={{ color: '#EE5D50' }}>
            <i className="fa-solid fa-arrow-trend-down"></i> TOP 5 Terendah - DPK
          </div>
          <ul className="top5-list">{renderRankingList(topRankings.dpkLow, '#ee5d50', false)}</ul>
        </div>
      </div>
    </div>
  );
